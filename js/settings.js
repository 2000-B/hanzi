...
function syncSettingsUI() {
  ...
  // Phase 6: tone pitch visualizer — Mandarin only; hide the rows entirely in JP mode
  ...
  const quickPitchRow = document.getElementById('quick-pitch-row');
  if (quickPitchRow) quickPitchRow.style.display = currentLang === 'zh' ? '' : 'none';
  const quickPitchToggle = document.getElementById('pitch-toggle');
  if (quickPitchToggle) quickPitchToggle.classList.toggle('on', toneGlyphsOnCard);
}
...
// ── Phase 6: master pitch-visualizer toggle (info panel + card face) ──
function toggleToneGlyphsOnCard() {
  ...
  // Re-render info panel so its tone line appears/disappears too.
  if (typeof renderInfoPanel === 'function' && activeDeck.length && infoPanelOpen) {
