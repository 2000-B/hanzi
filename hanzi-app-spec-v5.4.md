...
*v5.4 · March 2026 | Chinese (primary) · Japanese (planned)*
...
- Right-click any custom deck for a context menu (currently: delete)
- Saved to localStorage — survive closing and reopening the app

### 4.3.1 Favoriting *(new in v5.4)*

Any HSK chunk can be **favorited** via:
- The ★ star icon on each chunk row in the sidebar
- Right-clicking a chunk and selecting "Add to my decks" from the context menu

Favorited chunks appear in the **My Decks** section of the sidebar with a ★ tag badge, providing quick access without navigating through the HSK accordion. Favorites are persisted to `localStorage` under the `hanziFavorites` key as a JSON array of deck name strings.

### 4.3.2 Context Menu *(new in v5.4)*

Right-clicking any chunk item or custom deck shows a styled context menu at the cursor position. This is the extensible surface for future deck operations.

- **HSK chunks:** "Add to my decks" / "Remove from my decks" (toggles favorite)
- **Custom decks:** "Delete deck"
- The menu animates in (scale 0.95→1, opacity 0→1, 120ms ease) and dismisses on click-away.
...
- **Auto-text-shrink** *(new in v5.4):* Hanzi and English text automatically shrink when content is too long for the card face. Hanzi: >4 chars scales down to `max(28px, 72×3/length)`, >2 chars to `max(40px, 72×2/length)`. English: >40 chars → 16px, >25 chars → 20px.
- **Card face styling (dark mode):** Single smooth gradient (`linear-gradient(165deg, ...)`) instead of layered gradients, eliminating visible banding/horizontal line artifacts. Subtle radial sheen overlay via `::before`.
- **Side nav arrows:** `rgba(0,0,0,0.5)` background with `rgba(255,255,255,0.15)` hover, ensuring visibility against any background color (including solid custom backgrounds).
...
- **Undo** button — appears briefly (5s) after marking a card mastered

In test mode, the tray is not shown. A standalone **Shuffle** button appears in its place (no tray needed for a single control).

**Styling** *(updated in v5.4):*
- The chevron uses `color: var(--text2)` at `opacity: 0.7`, brightening to `var(--text)` at full opacity on hover.
- Tray buttons in dark mode use white text/icons with `rgba(0,0,0,0.4)` background and `rgba(255,255,255,0.2)` borders, ensuring visibility against any background color.
- Light mode uses standard theme colors (`var(--text)`, `var(--bg3)`, `var(--border2)`).
...
**Overlay behavior** *(new in v5.4):* The info panel is absolutely positioned within the split-pane container (right: 6px, top: 6px, bottom: 6px, z-index: 10). It **overlays** the main content rather than pushing the flashcard, so the card does not resize or shift when the panel opens. The card only needs to accommodate the panel if the viewport is narrow enough for overlap.

**Click-outside dismiss** *(new in v5.4):* Clicking anywhere outside the info panel dismisses it, **except** when clicking on: the sidebar/deck panel, the search bar/icon, the settings gear icon, or the info toggle button itself. This allows natural workflow transitions without accidentally closing the panel.

**Expanded mode** *(new in v5.4):* A "more info" button at the bottom of the info panel expands it to fill the entire main content area (position: absolute, inset: 0). The sidebar remains visible. Content is centered with a max-width of 720px for readability. The button label changes to "collapse" when expanded. This mode is intended for deep-diving into character details.

**Focused mode** *(new in v5.4):* Double-clicking the ⓘ info button activates focused study mode:
- The info panel opens (if not already open)
- The context strip is hidden (`display: none`)
- The side navigation arrows are hidden
- The card centers between the collapsed sidebar and the info panel
- Exiting: click outside the info panel, press Escape, or double-click the ⓘ button again
...
8. **More info button** *(new in v5.4):* A full-width button at the bottom of the scrollable content. Clicking expands the info panel to fill the main content area (see expanded mode above). Shows "collapse" when expanded, "more info" when normal.
...
| **Info panel** | Right side, absolutely positioned overlay (does not push flashcard). Unified reference + tutor (§7). Panel collapses fully via ⓘ toggle. Expandable to full main-content area via "more info" button. Click-outside dismisses (with exceptions). NOT dimmed when deck is active. |
...
| `hanziFavorites` | JSON array of favorited deck name strings (e.g. `["hsk 1 · 1/8", "hsk 2 · 3/7"]`). Controls which HSK chunks appear in the "my decks" sidebar section. *(new in v5.4)* |
...
| HSK 3–6 flashcards | ✓ done | Loaded async from separate JSON files |
| Study mode (flip) | ✓ done | Front/back, pinyin toggle, keyboard nav, auto-text-shrink |
| Test mode (MC + Typing) | ✓ done | Multiple choice + fuzzy typing match |
| Timer | ✓ done | Toggleable 10s countdown |
| Timer display only | ✓ done | See timer without penalty for slow answers |
| Start test flow | ✓ done | Deck prompt → Start button → session begins |
| Mastery tracking | ✓ done | 3 consecutive correct + manual Hide/Show button |
...
| Offline dictionary | ✓ done | CEDICT entries (separate JSON, cached by service worker) |
| AI deep dive (fallback) | ✓ done | Components + etymology generated on-demand via API |
| AI tutor | ✓ done | Context-aware chat in info panel. History saved to localStorage. |
| Light/dark mode | ✓ done | Toggle in settings |
| PWA + service worker | ✓ done | Static PWA for offline caching. Deployed to GitHub Pages. |
| Data export/import | ✓ done | JSON export/import in Full Settings. |
| Info panel | ✓ done | Overlay panel with definitions, enriched data, AI fallback, notes, TTS, tutor. |
| Info panel overlay | ✓ done | Absolutely positioned; does not push/resize flashcard. |
| Info panel expand | ✓ done | "More info" button expands panel to full main-content area. |
| Info panel click-outside | ✓ done | Click outside dismisses (exceptions: sidebar, search, settings, info btn). |
| Focused study mode | ✓ done | Double-click ⓘ: hides arrows + context strip, centers card beside panel. |
| Info panel resize | ✓ done | Drag left edge to resize (280–600px range). |
| Info panel audio | ✓ done | TTS via Web Speech API. Button hidden when no audio available. |
| User note field | ✓ done | Freeform per-card note in info panel. ✏ indicator on card front. |
| Mastery (Hide/Show) | ✓ done | Contextual label. SM-2 tracking begins on mastery. |
| Mobile responsive layout | ✓ done | Single-column below 768px. Swipe gestures for cards. |
| Language selector (stub) | ✓ done | Chinese / Japanese pill. Japanese shows 'coming soon'. |
| Animated gradient background | ✓ done | Pulsing blobs on black/white base with adjustable tint. |
| Custom background (image/color) | ✓ done | Upload image or pick solid color. Blur toggle for images. |
| Accent color adaptation | ✓ done | Full accent remap from background's dominant hue. |
| Settings restructure | ✓ done | Quick preferences popover + full settings modal. |
| Deck panel interaction | ✓ done | Hover-expand, click-deck-for-chunks, collapse behavior. |
| Card list overlay | ✓ done | Scrollable list inside flashcard with column alignment. Blurs card. |
| Bottom controls tray | ✓ done | Collapsible panel with shuffle, card list, hide/show, undo. White icons in dark mode. |
| Context strip | ✓ done | ~13 hanzi tiles above card. Click to navigate. Mastered tiles dimmed. |
| Search | ✓ done | Hanzi + tone-insensitive pinyin + English. Navigates to card + opens info panel. |
| Spaced repetition (SM-2) | ✓ done | Powers review deck. Activates on mastery; runs during review sessions. |
| Review deck | ✓ done | Sidebar button with count badge. Opens test mode with SM-2 ratings. |
| Difficulty ratings (test) | ✓ done | SM-2 Again/Hard/Good/Easy in test mode. Toggleable, off by default. |
| Related characters index | ✓ done | sameRadical lookup in enriched data. |
| Deck favoriting | ✓ done | Star icon on chunks + right-click context menu. Favorites appear in My Decks. |
| Context menu | ✓ done | Right-click on chunks (favorite) and custom decks (delete). Extensible. |
| Card auto-text-shrink | ✓ done | Hanzi and English auto-size based on content length. |
| Anti-aliasing | ✓ done | webkit-font-smoothing, GPU compositing on key elements. |
| Enriched offline char data | ✓ done | HSK 1–2 character + word enrichment. hsk-enriched.json shipped. |
| HSK 1 pre-recorded audio | 🟡 planned | ~152 audio files. Source TBD. TTS fallback working. |
| Japanese support (JLPT N5) | ⬜ 3–6 months | Separate data pipeline; shares app architecture. |
...
- **Anti-aliasing:** Body uses `-webkit-font-smoothing: antialiased` and `-moz-osx-font-smoothing: grayscale`. Key UI elements (sidebar, info panel, card faces, buttons, pills) use `backface-visibility: hidden` for GPU-composited rendering and smoother rounded corners.
